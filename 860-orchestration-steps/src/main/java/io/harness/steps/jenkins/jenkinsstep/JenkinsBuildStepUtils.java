...
@UtilityClass
public class JenkinsBuildStepUtils {
  public Map<String, ParameterField<String>> processJenkinsFieldsList(List<JenkinsParameterField> fields) {
    if (fields == null) {
      return null;
    }
    ...
    if (EmptyPredicate.isNotEmpty(duplicateFields)) {
      throw new InvalidRequestException(
          String.format("Duplicate jenkins fields: [%s]", String.join(", ", duplicateFields)));
    }
    return fieldsMap;
  }

  public Map<String, String> processJenkinsFieldsInParameters(Map<String, ParameterField<String>> fields) {
    ...
    for (Map.Entry<String, ParameterField<String>> entry : fields.entrySet()) {
      ...
      if (entry.getValue().isExpression()) {
        throw new InvalidRequestException(String.format("Field [%s] has invalid jenkins field value", entry.getKey()));
      }
