  ...
  public String getAcrRefreshToken(String registryUrl, String azureAccessToken) {
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        log.debug(format("Using access token %s", azureAccessToken));
      }
      ...
      if (response.isSuccessful()) {
        ...
        if (log.isDebugEnabled()) {
          log.debug(format("Retrieved Refresh token: %s", refreshToken));
        }
        return refreshToken;
      } else {
  ...
  public String getAcrAccessToken(String registryUrl, String azureAccessToken, String scope) {
    ...
    try {
      ...
      if (response.isSuccessful()) {
        ...
        if (log.isDebugEnabled()) {
          log.debug(format("Retrieved ACR access token: %s", acrAccessToken));
        }
