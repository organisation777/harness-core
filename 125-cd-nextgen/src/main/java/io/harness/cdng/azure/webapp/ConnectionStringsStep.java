...
public class ConnectionStringsStep implements SyncExecutable<ConnectionStringsParameters> {
  ...
  private void validateFileRefs(HarnessStore harnessStore, Ambiance ambiance) {
    ...
    if (fileReferences.size() > 1) {
      throw new InvalidRequestException(
          format("Only one connection string file should be provided, store kind: %s", harnessStore.getKind()));
    }
  ...
  private void validateFileByRef(HarnessStore harnessStore, Ambiance ambiance, HarnessStoreFile file) {
    ...
    if (ParameterFieldHelper.getBooleanParameterFieldValue(file.getIsEncrypted()) == Boolean.TRUE) {
      ...
      if (ngEncryptedData == null) {
        throw new InvalidRequestException(
            format("Connection strings not found in Encrypted Store with ref: [%s]", file.getRef()));
      }
    } else {
      ...
      if (!connectionStrings.isPresent()) {
        throw new InvalidRequestException(
            format("Connection strings not found in File Store with ref: [%s]", file.getRef()));
      }
    }
  }
